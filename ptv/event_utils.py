...
def gen_feature_event(events, events_val, vol_size):
    ...
    x = events[:, 0].long()
    y = events[:, 1].long()
    inds = (y * vol_size[2] + x).long()
    events_val = events_val.permute(1,0)
    event_feature = event_feature.view(vol_size[0], -1)
    event_feature.scatter_(1, (inds).unsqueeze(0).expand_as(events_val), events_val)
    event_feature = event_feature.view(vol_size)
    return event_feature
#输入
def gen_feature_dense_event(events, events_val, vol_size):
    events_cords = []
    events_feats = []
    for i in range(events.shape[0]):
        events_now = events[i]
        events_feat_now = events_val[i]
        zeros_channel = (torch.ones(events_now.shape[0], 1) * i).to(events_now.device)
        events_now = torch.cat((zeros_channel, events_now[:, :2]), dim=1)#只取了xy坐标
        events_cords.append(events_now)
        events_feats.append(events_feat_now)

    events_cords = torch.cat(events_cords, 0).int()
    events_feats = torch.cat(events_feats, 0)
    batch_size = len(torch.unique(events_cords[:, 0]))
    feat = spconv.SparseConvTensor(events_feats, events_cords, vol_size[1:], batch_size)
    feat = feat.dense()
    feat = F.interpolate(feat, size=(224, 224), mode='bilinear', align_corners=False)
    return feat

def gen_feature_dense_event2(events, events_val, vol_size):
    zeros_channel = torch.zeros(events.shape[0], 1).to(events.device)
    events = torch.cat((zeros_channel, events[:, :2]), dim=1)
    events = events.int()
    feat = spconv.SparseConvTensor(events_val, events, vol_size[1:], 1)
    feat = feat.dense()
    feat = F.interpolate(feat, size=(224, 224), mode='bilinear', align_corners=True)
    return feat

def gen_feature_dense_event_3(events, events_val, vol_size):
    fin_feat = []
    for i in range(events.shape[0]):
        events_now = events[i]
        features = events_val[i]
        xy_coord = events_now[:, :2]
        unique_xy, inverse_indices = torch.unique(xy_coord, dim=0, return_inverse=True)
        aggregated_features = torch.zeros(unique_xy.size(0), features.size(1), device=events.device)
        counts = torch.bincount(inverse_indices)
        aggregated_features.index_add_(0, inverse_indices, features)
        aggregated_features /= counts.unsqueeze(1)
        
        output_array = torch.zeros(vol_size[0], vol_size[1], aggregated_features.size(1), device=events.device)
        output_array = output_array.view(-1, aggregated_features.size(1))
        unique_xy = unique_xy.to(torch.int32)
        output_array.index_add_(0, unique_xy[:, 1] * vol_size[1] + unique_xy[:, 0], aggregated_features)
        output_array = output_array.view(vol_size[0], vol_size[1], aggregated_features.size(1))
        fin_feat.append(output_array)
    fin_feat = torch.stack(fin_feat).permute(0, 3, 1, 2)
    fin_feat = F.interpolate(fin_feat, size=(224, 224), mode='bilinear', align_corners=True)
    return fin_feat

def gen_feature_dense_event_4(events_ori, events_ptv3, vol_size, batch):
    fin_feat = []
    for i in range(batch[-1]+1):
        idx = idx = torch.where(batch == i)[0]
        events_now = events_ori[idx]
        features = events_ptv3[idx]
        xy_coord = events_now[:, :2]
        unique_xy, inverse_indices = torch.unique(xy_coord, dim=0, return_inverse=True)
        aggregated_features = torch.zeros(unique_xy.size(0), features.size(1), device=events_ori.device)
        counts = torch.bincount(inverse_indices)
        aggregated_features.index_add_(0, inverse_indices, features)
        aggregated_features /= counts.unsqueeze(1)
        
        output_array = torch.zeros(vol_size[0], vol_size[1], aggregated_features.size(1), device=events_ori.device)
        output_array = output_array.view(-1, aggregated_features.size(1))
        unique_xy = unique_xy.to(torch.int32)
        output_array.index_add_(0, unique_xy[:, 1] * vol_size[1] + unique_xy[:, 0], aggregated_features)
        output_array = output_array.view(vol_size[0], vol_size[1], aggregated_features.size(1))
