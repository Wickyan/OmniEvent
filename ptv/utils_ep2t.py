...
def query_ball_point_weight(radius, nsample, xyz, new_xyz, t_list, weight):
    ...
    max_t = (max_t[:, 0]).unsqueeze(-1).unsqueeze(-1)
    theld = ((32/256.) + (32/256.))* weight[0] + max_t * weight[1]
    for batch in range(B):
        xyz_now = xyz[batch]
        new_xyz_now = new_xyz[batch]
        xyz_now = transform(xyz_now, weight)
        new_xyz_now = transform(new_xyz_now, weight)
        nn = NearestNeighbors(n_neighbors=nsample, algorithm='kd_tree', metric='l1')
        nn.fit(xyz_now)
        distances, indices = nn.kneighbors(new_xyz_now)
        distances = torch.from_numpy(distances).to(xyz.device)
        indices = torch.from_numpy(indices).to(xyz.device)
        print(distances)
        print(theld[batch])
        group_first = indices[:, 0].view(S, 1).repeat([1, nsample])
        distances[distances > theld[batch]] = N

        mask = distances[:,:nsample] == N
        masked_points_per_batch = torch.sum(mask, -1).float()
        average_masked_points_per_batch = torch.mean(masked_points_per_batch, -1)
        print(average_masked_points_per_batch)

        mask = distances[:,:nsample] == N
        group_idx = indices[:, :nsample]
        group_idx[mask] = group_first[mask]
        group_idxs.append(group_idx)
    group_idxs = torch.stack(group_idxs, 0).to(xyz.device)
    return group_idxs
...
def square_distance(src, dst, weight):
    ...
    dist = torch.sqrt(dist_xy) * weight[0] + torch.sqrt(dist_t) * weight[1] 
...
def query_ball_point(radius, nsample, xyz, new_xyz, t_list, weight):
    ...
    distances = t_list[:, 1:] - t_list[:, :-1]
    max_t = distances.sort(dim=-1, descending = True)[0]
    max_t = ((max_t[:, 0] ** 2) * 2).unsqueeze(-1).unsqueeze(-1)
...
def sample_and_group(radius, nsample, xyz, points, new_xyz, t_list, weight):
    ...
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
    # idx = query_ball_point_weight(radius, nsample, xyz, new_xyz, t_list, weight)
    # print(idx.shape)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            _, _, _, dists = square_distance(xyz1, xyz2, [1.0, 1.0])
