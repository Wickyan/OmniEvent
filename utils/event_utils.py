...
def per_stacking_events(events, events_val, vol_size, val_plus = False):
    ...
    event_stacking_images_temp = events.new_zeros(vol_size,dtype=torch.float32)
...
def per_event_counting_images(events, events_val, vol_size, val_plus = False):
    event_count_images = events.new_zeros(vol_size,dtype=torch.float32)
    x = events[:, 0].long()
    y = events[:, 1].long()
    t = events[:, 2]
    p = events[:, 3].long()
...
def gen_feature_dense_event_3(events, events_val, vol_size):
    ...
    for i in range(events.shape[0]):
        ...
        output_array = torch.zeros(vol_size[1], vol_size[2], aggregated_features.size(1), device=events.device)
        output_array = output_array.view(-1, aggregated_features.size(1))
        unique_xy = unique_xy.to(torch.int32)
        output_array.index_add_(0, unique_xy[:, 0] * vol_size[1] + unique_xy[:, 1], aggregated_features)
        output_array = output_array.view(vol_size[1], vol_size[2], aggregated_features.size(1))
